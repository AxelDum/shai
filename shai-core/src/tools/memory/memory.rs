...
use super::structs::{MemoryWriteParams, MemoryReadParams};
...
fn resolve_memory_file_path() -> PathBuf {
    ...
    let base = git_root.unwrap_or_else(|| {
...
fn append_memory_fact(content: &str) -> Result<(), String> {
    ...
    let mut file = fs::OpenOptions::new()
        ...
        .append(true)
        .open(&memory_path)
        .map_err(|e| format!("Failed to open memory file '{}': {}", memory_path.display(), e))?;
...
fn read_memory_facts() -> Result<String, String> {
    ...
    fs::read_to_string(&memory_path)
        .map_err(|e| format!("Failed to read memory file '{}': {}", memory_path.display(), e))
}
